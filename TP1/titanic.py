...
def splitDataset(dataSet, splitRatio):
    random.seed(123)
    trainSize = int(len(dataSet) * splitRatio)  # Number of elements kept
    trainingSet = []
    copy = list(dataSet)
    while len(trainingSet) < trainSize:
        index = random.randrange(len(copy)) # Get the index of one element
        trainingSet.append(copy.pop(index)) # Pop it from copy
    trainingSet = np.array(trainingSet)     # Transform in np.array
    return [trainingSet, copy]              # "copy" is the testing set
...
def getProba(attribut, Resultat, totalResul):
    ...
    # The computation of the probability is simply
    # n_i / n
    # where n_i is the cardinality of a_i
    # and n is the cardinality of a given class
    # e.g. : P(age = adu | class = dead) = nbr of adult dead / total of dead
    for att in attri:
        actu = attri[att]
        for res in Resultat:
            Proba[att][res] = {}
...
liste, debut = obtainData("titanic.csv")
...
# Get the probability of each classes P(c_j)
